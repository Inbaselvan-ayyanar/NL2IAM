...
const Group = () => {
  ...
  const handleDownload = async () => {
    if (apiStatus === "success" && storedFile) {
      try {
        const response = await fetch('http://127.0.0.1:5000/download/group_report.csv');
        if (response.ok) {
    ...
    <div className="group-container">
      <div className="header">
        <h1>
          ...
          AWS IAM Group Management Dashboard
        </h1>
      ...
      <div className="card main-card">
        <div className="card-header">
          ...
          <div className="card-title">AWS IAM Group Processing</div>
        </div>
